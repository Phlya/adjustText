...
def repel_text_annotations(texts, ax=None, expand=(1.2, 1.2), precision=0.1, prefer_move='xy', only_use_max_min=False):
    if ax is None:
        ax = plt.gca()
...
def repel_text_annotations_from_points(x, y, texts, ax=None, prefer_move='xy', expand=(1.2, 1.2),
...
def pull_text_annotations_to_respective_points(x, y, texts, fraction=0.1, expand=(1.2, 1.2)):
    bboxes = [i.get_window_extent(r).expanded(*expand).transformed(plt.gca().transData.inverted()) for i in texts]
    delta_x = []
    delta_y = []
...
def adjust_text(x, y, texts, expand_text = (1.2, 1.2), expand_points=(1.3, 1.3), prefer_move = 'y',
                                        lim=100, precision=0.1, pullback_fraction=0.2, only_use_max_min=False,
    ...
    for i in range(lim):
        q1, q2 = np.inf, np.inf
        if text_from_text:
            texts, q1 = repel_text_annotations(texts, expand=expand_text, precision=precision,
                                               prefer_move=prefer_move, only_use_max_min=only_use_max_min)
        if text_from_points:
            texts, q2 = repel_text_annotations_from_points(x, y, texts, expand=expand_points, prefer_move=prefer_move,
                                                           precision=precision, only_use_max_min=only_use_max_min)
        if pullback_fraction:
            tests = pull_text_annotations_to_respective_points(x, y, texts, fraction=pullback_fraction, expand=expand_points)
    ...
    for j, text in enumerate(texts):
        plt.annotate(text.get_text(), xy = (x[j], y[j]), xytext=text.get_position(), *args, **kwargs)
